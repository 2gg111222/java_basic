...
public class 스레드그래픽 extends JFrame{		//public class 스레드그래픽 extends JFrame{
	//add, setSize(),,,,,,,,
	JLabel count, image, time;
	public static void main(String[] args) {//public static void main(String[] args) {
		스레드그래픽 f = new 스레드그래픽();				//	f.call();
	...
	public 스레드그래픽() {
		...
		//내부클래스(inner class) ==> 전역변수 공유 할 목적으로사용
		//좋은 코드 아님
		public class 카운트스레드2 extends Thread {
			...
			public void run() {
				for (int i = 500; i > 0; i--) {
					...
					//thread를 잠깐 재울 수 있음.
					//cpu는 외부자원입니다.
					//자바가 외부자원을 연결 할 때는 엄청! 위험해!!라는  상황으로 인식
		...
		public class 이미지스레드2 extends Thread {
			...
			public void run() {
				...
				for (int i = 0; i < images.length; i++) {
					...
					try {
						Thread.sleep(5000); //5초 재워라!/ms(밀리세컨즈), 파이썬은 초단위 가능
					} catch (Exception e) {
						System.out.println("cpu 연결 문제 생김.");
					}
				}
